...
IUSE_CONTRIB="smbkrb5passwd kerberos passlastchange"
...
src_prepare() {
	...
	#marmarek
	epatch "${FILESDIR}"/${PN}-2.4.11-smbk5pwd-no_private_functions.patch

	if use passlastchange; then
		epatch "${FILESDIR}"/${PN}-2.4.11-smbk5pwd-shadowLastChange.patch
	fi
