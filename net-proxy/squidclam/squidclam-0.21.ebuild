...
src_unpack() {
	unpack ${A}
	cd ${S}
	sed -i -e 's$//#define ERRIGNORE$#define ERRIGNORE$' src/squidclam.c
	epatch ${FILESDIR}/${P}-url_rewrite.patch
}
...
pkg_postinst() {
	...
	einfo "${HILITE}    url_rewrite_program /usr/bin/squidclam ${NORMAL}"
	einfo "${HILITE}    url_rewrite_children 15 ${NORMAL} # make a wise choice"
	einfo "and this line to your acl list to prevent loops:"
	einfo "${HILITE}    url_rewrite_access deny localhost ${NORMAL}"
	echo
	...
	einfo "${HILITE}    url_rewrite_access deny SSL_ports ${NORMAL}"
	echo
}
