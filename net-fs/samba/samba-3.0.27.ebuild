...
src_compile() {
	...
	if use oav; then
		cd ${WORKDIR}/${VSCAN}
		econf --with-samba-source=${S}/source || die
		emake || die
	fi
}
...
src_install() {
	...
	if use oav; then
		...
		insinto /etc/samba
		doins */vscan-*.conf
		einfo "To enable vscan, add to smb.conf:"
		einfo "  vfs object = vscan-oav"
		einfo "  vscan-oav: config-file = /etc/samba/vscan-[backend].conf"
	fi
}
