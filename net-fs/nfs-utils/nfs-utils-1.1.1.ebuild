...
src_unpack() {
	...
	sed -i -e 's/typedef uint64_t OM_uint64/\/\/typedef uint64_t OM_uint64/' utils/gssd/context_lucid.c

	epatch ${FILESDIR}/${P}-heimdal.patch
	epatch ${FILESDIR}/${P}-krb5-gssapi.patch
	epatch ${FILESDIR}/${P}-no-libgssglue.patch
