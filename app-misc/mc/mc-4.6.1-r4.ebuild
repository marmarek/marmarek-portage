...
#	fi
	...
	#paches from linuxfromscratch
	if use unicode; then
		# modified http://www.linuxfromscratch.org/patches/blfs/svn/mc-4.6.1-debian_fixes-1.patch
		epatch "${FILESDIR}/mc-4.6.1-debian-fixed.patch"
...
src_compile() {
	...
	if use unicode; then
		CFLAGS="${CFLAGS} -DUTF8 -I/usr/include/slang-2"
		LDFLAGS="${LDFLAGS}"
	fi
