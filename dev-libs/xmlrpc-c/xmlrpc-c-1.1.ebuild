...
#	epatch "${FILESDIR}"/${PN}-1.05-pic.patch
	...
	epatch "${FILESDIR}/xmlrpc-c-1.1-xml_includes.patch"
	epatch "${FILESDIR}/xmlrpc-c-1.1-tools.patch"
	epatch "${FILESDIR}/xmlrpc-c-1.1-libxml2.patch"
}
...
src_compile() {
	...
	export LADD=${LDFLAGS}
	econf --disable-wininet-client --enable-libxml2-backend --enable-libwww-client \
...
src_install() {
	...
	dodoc README CREDITS SECURITY TESTING REFACTORINGS PORTING NEWS || die "installing docs failed"
}
