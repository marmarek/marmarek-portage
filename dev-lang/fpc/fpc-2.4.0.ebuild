...
src_compile() {
	...
	emake -j1 PP="${pp}" CPU_TARGET=${CPU_TARGET} compiler_cycle || die "make compiler_cycle failed!"
	...
	emake -j1 PP="${S}"/ppc${FPC_ARCH}.new CPU_TARGET=${CPU_TARGET} compiler_cycle || die "make compiler_cycle failed!"
	...
	emake -j1 PP="${pp}" CPU_TARGET=${CPU_TARGET} rtl_clean || die "make rtl_clean failed"

	emake -j1 PP="${pp}" CPU_TARGET=${CPU_TARGET} rtl packages_all utils || die "make failed"
}
